	...
	public class PluginManager : IDisposable
	{
		...
		public void LoadFrom(string pluginsPath, Type[] pluginInterfaces)
		{
			...
			try
			{
				...
				pluginFilesPaths = directories
					.Select(BuildPluginDllFileName)
					.Where(File.Exists)
					.ToArray();
			}
		...
		/// <summary>
		/// Builds full main assembly plugin file name from plugin directory path.
		/// </summary>
		/// <param name="pluginDirectory">The plugin directory path.</param>
		/// <returns>The main plugin assembly file path.</returns>
		private string BuildPluginDllFileName(string pluginDirectory)
		{
			return Path.Combine(pluginDirectory, Path.GetFileName(pluginDirectory) + ".dll");
		}
	}
}
